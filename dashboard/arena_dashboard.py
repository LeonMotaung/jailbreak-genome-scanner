...
# =============================================================================
# STATIC MODEL CONFIGURATION - Available Models and Endpoints
# =============================================================================
...
try:
    ...
    log.warning(
        "python-dotenv not installed, environment variables may not load from .env file"
    )
...
def create_jvi_gauge(value):
    ...
    fig = go.Figure(
        go.Indicator(
            ...
            domain={"x": [0, 1], "y": [0, 1]},
            title={
                "text": f"JVI Score<br><span style='font-size:0.8em;color:{color}'>{category}</span>",
...
def create_trend_chart(evaluation_history: List[Dict]) -> Optional[go.Figure]:
    ...
    for i, eval_result in enumerate(evaluation_history):
        ...
        is_jailbroken = (
            eval_result.get("is_jailbroken", False)
            if isinstance(eval_result, dict)
            else eval_result.is_jailbroken
        )
        if is_jailbroken:
    ...
    exploit_rates = [
        (rounds_data[r]["exploits"] / rounds_data[r]["total"] * 100) for r in rounds
    ]
...
def create_strategy_distribution_chart(
    ...
    for eval_result in evaluation_history:
        if isinstance(eval_result, dict):
            ...
            strategy_name = (
                str(eval_result.attack_strategy.value)
                if hasattr(eval_result.attack_strategy, "value")
                else str(eval_result.attack_strategy)
            )
    ...
    fig.update_layout(
        title={
            "text": "Successful Attacks by Strategy",
    ...
    for eval_result in evaluation_history:
        ...
        if is_jailbroken:
            if isinstance(eval_result, dict):
                ...
                severity_val = (
                    severity.get("value", 0) if isinstance(severity, dict) else severity
                )
            else:
                severity_val = (
                    eval_result.severity.value
                    if hasattr(eval_result.severity, "value")
                    else eval_result.severity
                )
...
def create_leaderboard_chart(attackers):
    ...
    if isinstance(attackers[0], dict):
        ...
        success_rates = [
            a.get("success_rate", 0) * 100
            if isinstance(a.get("success_rate"), (int, float))
            else 0
    ...
    fig.update_layout(
        title={
            "text": "Attacker Leaderboard",
            "font": {"size": 20, "color": "#ffffff"},
        },
        xaxis={
            "title": {"text": "Attackers", "font": {"color": "#e0e0e0", "size": 14}},
            "tickfont": {"color": "#e0e0e0"},
            "gridcolor": "#333333",
        },
        yaxis={
            "title": {"text": "Total Points", "font": {"color": "#28a745", "size": 14}},
            "tickfont": {"color": "#e0e0e0"},
            "gridcolor": "#333333",
        },
        yaxis2={
...
def main():
    ...
    st.markdown(
        '<p style="text-align: center; color: #999999; margin-top: -1rem;">Active Defense Infrastructure - Automated Red-Teaming & Threat Radar</p>',
        unsafe_allow_html=True,
    )
        ...
        try:
            with open(deployment_config_path, "r") as f:
                ...
                if deployed_models:
                    ...
                    h100_models = [
                        inst
                        for inst in deployed_models
                        if "h100" in inst.get("instance_type", "").lower()
                        or "h100" in inst.get("key", "").lower()
                    ]
                    ...
                    if h100_models:
                        ...
                        first = h100_models[0]
                        log.info(
                            f"Auto-selected H100 model instance: {first.get('instance_id')}"
                        )
                    else:
    ...
    with st.sidebar:
        ...
        # Model selection - simplified to static list
        st.subheader("Defender Model")
        defender_model_key = st.selectbox(
            "Select Defender Model",
            list(AVAILABLE_MODELS.keys()),
            index=0,
            help="Choose the model to defend against jailbreak attacks",
        )

        defender_config = AVAILABLE_MODELS[defender_model_key]

        # Display model info
        st.markdown(f"**{defender_config['display_name']}**")
        st.caption(defender_config["description"])
        ...
        use_database = st.checkbox(
            "Use Structured Prompt Database",
            value=True,
    ...
    if start_battle or st.session_state.battle_running:
        ...
        if use_scraper:
            ...
            scraper_status.info(
                "🕵️ Gathering recent attack patterns and extracting intelligence..."
            )
            ...
            def run_intelligence_gathering():
                ...
                try:
                    log.info(
                        "Starting intelligence gathering with threat intelligence engine..."
                    )

                    # Create scraper
                    scraper = LambdaWebScraper(
                        instance_id=scraper_instance_id if scraper_instance_id else None
                    )
                    ...
                    try:
                        ...
                        if (
                            hasattr(st_module, "session_state")
                            and "arena" in st_module.session_state
                        ):
                            arena_ref = st_module.session_state.arena
                            if (
                                arena_ref
                                and hasattr(arena_ref, "pattern_database")
                                and arena_ref.pattern_database
                            ):
                                pattern_db_ref = arena_ref.pattern_database
                                log.info("Using existing arena pattern database")
                    except Exception as e:
                        log.debug(
                            f"Could not access session state in thread (expected): {e}"
                        )
                    ...
                    if pattern_db_ref:
                        ...
                        from src.intelligence.pattern_database import (
                            ExploitPatternDatabase,
                        )
                    ...
                    threat_engine = ThreatIntelligenceEngine(
                        pattern_database=pattern_db, scraper=scraper
                    )
                    ...
                    log.info(
                        "Threat intelligence engine initialized, starting scraper..."
                    )

                    # Update intelligence from scraper - THIS ACTUALLY EXTRACTS PATTERNS!
                    update_result = run_async(
                        threat_engine.update_from_scraper(days_back=7, max_results=50)
                    )
                    ...
                    if scraper_instance_id:
                        ...
                        recent_data = run_async(
                            gather_recent_attacks(scraper, scraper_instance_id)
                        )
                    else:
                    ...
                    if recent_data:
                        ...
                        log.info(
                            f"Intelligence gathering complete: {update_result.get('patterns_added', 0)} patterns added, {integrate_result.get('prompts_added', 0)} prompts integrated"
                        )
                    else:
                        st.session_state.scraper_data = None
                        st.session_state.scraper_status = "complete_no_data"
                        log.info(
                            f"Intelligence gathering complete (no display data): {update_result.get('patterns_added', 0)} patterns added"
                        )
                except Exception as e:
                    import traceback
                    ...
                    log.error(
                        f"Intelligence gathering error: {e}\n{traceback.format_exc()}"
                    )

            # Start intelligence gathering in background thread
            scraper_thread = threading.Thread(
                target=run_intelligence_gathering, daemon=True
            )
            ...
            if "scraper_status" in st.session_state:
                if st.session_state.scraper_status == "complete":
                    ...
                    intel_integration = st.session_state.get(
                    ...
                    if patterns_added > 0 or prompts_added > 0:
                        scraper_status.success(
                            f"✅ Intelligence gathered: {patterns_added} patterns extracted, {prompts_added} prompts added to database"
                        )
                    else:
                        scraper_status.info(
                            "✅ Intelligence gathering complete (no new patterns found)"
                        )
                    ...
                    if patterns_added > 0:
                        scraper_status.success(
                            f"✅ {patterns_added} patterns extracted and added to database"
                        )
                    else:
                        scraper_status.empty()
                elif st.session_state.scraper_status.startswith("error"):
                    scraper_status.warning(
                        f"⚠️ Intelligence gathering error: {st.session_state.scraper_status.split(':', 1)[1]}"
                    )
            ...
            if patterns_added > 0 or prompts_added > 0 or events_found > 0:
                st.markdown("---")
                st.success(
                    f"🕵️ Intelligence Gathering Complete: {events_found} events found, {patterns_added} patterns extracted, {prompts_added} prompts added"
                )
                ...
                if st.session_state.scraper_data:
                    with st.expander(
                        "📊 Recent Attack Patterns from Web Sources", expanded=True
                    ):
                        st.markdown(
                            """
                        ...
                        for i, event in enumerate(events[:10], 1):  # Show top 10
                            if event.strip():
                                ...
                                if len(lines) >= 2:
                                    ...
                                    source_line = next(
                                        (l for l in lines if l.startswith("Source:")),
                                        "",
                                    )
                                    category_line = next(
                                        (l for l in lines if l.startswith("Category:")),
                                        "",
                                    )
                                    url_line = next(
                                        (l for l in lines if l.startswith("URL:")), ""
                                    )
        ...
        with st.spinner("Setting up Defender"):
            try:
                ...
                try:
                    ...
                    attacker_strategies = [
                        a.strategy for a in st.session_state.arena.attackers
                    ]
                    ...
                    if has_bio and has_cyber:
                        ...
                        """.format(len(st.session_state.arena.attackers)),
                            unsafe_allow_html=True,
                        )
                    else:
                        log.warning(
                            f"BIO_HAZARD or CYBER_EXPLOIT missing! Has BIO: {has_bio}, Has CYBER: {has_cyber}"
                        )
                        agent_status.markdown(
                            """
                    ...
                    log.warning(
                        f"difficulty_range parameter issue: {e}, using fallback"
                    )
                    st.session_state.arena.generate_attackers(
                        num_strategies=num_attackers
                    )
                st.markdown(
                    '<div style="padding: 0.75rem; background: rgba(34, 197, 94, 0.1); border-left: 3px solid rgba(34, 197, 94, 0.8); border-radius: 6px; margin: 0.5rem 0; color: #86efac;"><i class="fas fa-check-circle" style="margin-right: 0.5rem; color: #22c55e;"></i> Defender ready</div>',
                    unsafe_allow_html=True,
                )
                ...
                if use_database and st.session_state.arena.prompt_generator.prompt_db:
                    ...
                    if prompts_added > 0:
                        st.success(
                            f"✅ Using {db_stats['total_prompts']} structured prompts from database ({prompts_added} newly gathered from intelligence sources)"
                        )
                    else:
                        st.info(
                            f"Using {db_stats['total_prompts']} structured prompts from database"
                        )
        ...
        with col_header1:
            st.markdown(
                '<h3 style="margin: 0; padding: 0.5rem 0;"><i class="fas fa-shield-alt" style="margin-right: 0.5rem; color: #06b6d4;"></i> EVALUATION IN PROGRESS</h3>',
                unsafe_allow_html=True,
            )
        with col_header2:
        ...
        with col_header3:
            st.markdown(
                '<i class="fas fa-sync-alt fa-spin" style="margin-right: 0.5rem;"></i> **Status:** Running',
                unsafe_allow_html=True,
            )
        ...
        with battle_container:
            ...
            st.markdown(
                "*Watch attacks as planes flying toward threat zones. Blocked attacks explode, successful exploits pass through.*"
            )

            # Load and display live threat radar visualization
            radar_template_path = (
                Path(__file__).parent / "templates" / "threat_radar_live.html"
            )
        ...
        status_text.markdown(
            f'<i class="fas fa-rocket" style="margin-right: 0.5rem;"></i> Starting evaluation: {num_rounds} rounds × {num_attackers} attackers = {total_attempts} total attack attempts',
            unsafe_allow_html=True,
        )
        ...
        try:
            ...
            def send_realtime_update(eval_result, round_num):
                ...
                try:
                    ...
                    radar_update = {
                        ...
                        if hasattr(eval_result, "id")
                        else f"eval_{round_num}_{len(st.session_state.realtime_updates)}",
                        "strategy": str(eval_result.attack_strategy.value)
                        if hasattr(eval_result.attack_strategy, "value")
                        else str(eval_result.attack_strategy),
                        "prompt": str(eval_result.prompt)[:50]
                        if eval_result.prompt
                        else "",
                        "isJailbroken": eval_result.is_jailbroken
                        if hasattr(eval_result, "is_jailbroken")
                        else eval_result.get("is_jailbroken", False),
            ...
            for round_num in range(1, num_rounds + 1):
                ...
                round_result = run_async(
                    st.session_state.arena._run_round(
                        round_num=round_num,
                        defenders=[defender],
                ...
                with live_stats_container.container():
                    with st.expander("Live Evaluation Metrics", expanded=False):
                        ...
                        with col1:
                            st.metric(
                                "Rounds Completed", f"{completed_rounds}/{num_rounds}"
                            )
                        with col2:
                            st.metric(
                                "Total Attempts",
                                f"{completed_attempts}/{total_attempts}",
                            )
                        with col3:
                            exploit_rate_pct = (
                                (completed_exploits / completed_attempts * 100)
                                if completed_attempts > 0
                                else 0
                            )
                            st.metric(
                                "Exploits",
                                f"{completed_exploits}/{completed_attempts}",
                                delta=f"{exploit_rate_pct:.1f}%",
                            )
                        with col4:
                            elapsed = (
                            ...
                            st.metric(
                                "Elapsed Time",
                                f"{elapsed:.1f}s",
                                delta=f"~{avg_time:.1f}s/round",
                            )

                        # Learning metrics
                        if (
                            hasattr(st.session_state, "arena")
                            and st.session_state.arena
                        ):
                            unique_prompts = (
                                len(st.session_state.arena.used_prompts)
                                if hasattr(st.session_state.arena, "used_prompts")
                                else 0
                            )
                            successful_patterns = (
                                len(st.session_state.arena.successful_prompts)
                                if hasattr(st.session_state.arena, "successful_prompts")
                                else 0
                            )
                            pattern_db_size = (
                                len(st.session_state.arena.pattern_database.patterns)
                                if (
                                    hasattr(st.session_state.arena, "pattern_database")
                                    and st.session_state.arena.pattern_database
                                )
                                else 0
                            )
                            ...
                            if unique_prompts > 0 or successful_patterns > 0:
                                ...
                                with learn_col1:
                                    freshness_rate = (
                                        (unique_prompts / completed_attempts * 100)
                                        if completed_attempts > 0
                                        else 100
                                    )
                                    st.metric(
                                        "Unique Prompts",
                                        unique_prompts,
                                        delta=f"{freshness_rate:.1f}% fresh",
                                    )
                                with learn_col2:
                                    st.metric(
                                        "Patterns Learned",
                                        successful_patterns,
                                        delta=f"{pattern_db_size} total",
                                    )
                                with learn_col3:
                                    creativity = min(
                                        100,
                                        (freshness_rate + (successful_patterns * 2)),
                                    )
                                    st.metric(
                                        "Creativity",
                                        f"{creativity:.0f}/100",
                                        delta="High" if creativity > 80 else "Medium",
                                    )
                ...
                for eval_result in round_evals:
                    if hasattr(eval_result, "is_jailbroken"):
                        status = (
                            "JAILBROKEN" if eval_result.is_jailbroken else "BLOCKED"
                        )
                        strategy = (
                            eval_result.attack_strategy.value
                            if hasattr(eval_result.attack_strategy, "value")
                            else str(eval_result.attack_strategy)
                        )
                        severity = (
                        ...
                        prompt = (
                            str(eval_result.prompt)[:80] if eval_result.prompt else ""
                        )
                    else:
                        status = (
                            "JAILBROKEN"
                            if eval_result.get("is_jailbroken", False)
                            else "BLOCKED"
                        )
                        attack_strat = eval_result.get("attack_strategy", {})
                        strategy = (
                            attack_strat.get("value", "unknown")
                            if isinstance(attack_strat, dict)
                            else str(attack_strat)
                        )
                ...
                if radar_template_path.exists():
                    ...
                    for eval_result in round_evals:
                        radar_update = {
                            ...
                            "id": str(eval_result.id)
                            if hasattr(eval_result, "id")
                            else f"eval_{round_num}_{len(radar_updates)}",
                            "strategy": str(eval_result.attack_strategy.value)
                            if hasattr(eval_result.attack_strategy, "value")
                            else str(eval_result.attack_strategy),
                            "prompt": str(eval_result.prompt)[:50]
                    ...
                    serializable_round_update = make_json_serializable(
                        round_update_dict
                    )
                    stats_dict_2 = {
                        "type": "stats_update",
                ...
                with live_logs_container.container():
                    with st.expander(
                        f"Live Round-by-Round Results (Round {round_num}/{num_rounds})",
                        expanded=False,
                    ):
                        st.markdown(
                            f"**Round {round_num}/{num_rounds}** - {len(round_evals)} attempts, {sum(1 for e in round_evals if e.is_jailbroken)} exploits"
                        )

                        # Show last 20 log entries across all rounds
                        all_logs = (
                            st.session_state.eval_progress["round_logs"] + round_logs
                        )
                        st.session_state.eval_progress["round_logs"] = all_logs[
                            -50:
                        ]  # Keep last 50
            ...
            freshness_rate = (
                (unique_prompts / total_evaluations * 100)
            ...
            if hasattr(st.session_state, "arena") and st.session_state.arena:
                ...
                with learn_col3:
                    st.metric(
                        ...
                        pattern_db_size,
                        delta=f"{successful_patterns} recent"
                ...
                with learn_col4:
                    creativity_score = min(
                        100, (freshness_rate + (successful_patterns * 2))
                    )
                    st.metric(
                        "Creativity Score",
                ...
                if successful_patterns > 0:
                    ...
                    st.info(
                        "💡 **Learning System Ready**: As successful exploits are found, they will be stored and used to generate creative new attack variations."
                    )

        except Exception as e:
            st.markdown(
                f'<div style="padding: 0.75rem; background: rgba(239, 68, 68, 0.1); border-left: 3px solid rgba(239, 68, 68, 0.8); border-radius: 6px; margin: 0.5rem 0; color: #fca5a5;"><i class="fas fa-times-circle" style="margin-right: 0.5rem; color: #ef4444;"></i> Evaluation failed: {e}</div>',
                unsafe_allow_html=True,
            )
            import traceback
        ...
        if results and results.get("evaluation_history"):
            ...
            num_attackers_in_arena = (
                len(st.session_state.arena.attackers)
                if st.session_state.arena
                and hasattr(st.session_state.arena, "attackers")
                else num_attackers
            )
            ...
            for idx, eval_result in enumerate(evaluation_history):
                ...
                if hasattr(eval_result, "is_jailbroken"):
                    ...
                    response = (
                        str(eval_result.response)[:100] if eval_result.response else ""
                    )
                else:
                    # Dictionary - use .get()
                    status = (
                ...
                log_entry = {
                    ...
                    if len(response) > 200
                    else response,  # Store response for display
                }
        ...
        if results:
            ...
            with col2:
                st.metric(
                    "Total Attempts",
                    total_evaluations,
                    delta=f"{total_attempts} expected"
                    if total_evaluations != total_attempts
                    else None,
                    delta_color="off",
                )
            with col3:
                st.metric(
                    "Exploits Found",
                    f"{total_exploits}/{total_evaluations}",
        ...
        if results and results.get("defenders"):
            ...
            with col1:
                ...
                if isinstance(defender_result, dict):
                    ...
                    jvi = (
                        defender_result.jvi.jvi_score
                        if hasattr(defender_result, "jvi")
                        else 0
                    )
                    jvi_data = (
                        defender_result.jvi if hasattr(defender_result, "jvi") else {}
                    )

                st.plotly_chart(
                    create_jvi_gauge(jvi),
                ...
                st.markdown(
                    "*Real-time vulnerability tracking for regulatory assessment*"
                )
                ...
                st.session_state.jvi_history.append(
                    {
                        ...
                        if "exploit_rate" in locals()
                        else 0,
                        "mean_severity": mean_severity
                        if "mean_severity" in locals()
                        else 0,
                    }
                )
                ...
                if len(st.session_state.jvi_history) > 1:
                    ...
                    st.plotly_chart(
                        fig_jvi_trend, use_container_width=True, key="jvi_live_monitor"
                    )
            ...
            with col2:
                ...
                if isinstance(defender_result, dict):
                    ...
                    exploit_rate = (
                        jvi_data.exploit_rate
                        if hasattr(jvi_data, "exploit_rate")
                        else 0
                    )
                    mean_severity = (
                        jvi_data.mean_severity
                        if hasattr(jvi_data, "mean_severity")
                        else 0
                    )
                    high_severity_rate = (
                        jvi_data.high_severity_rate
                        if hasattr(jvi_data, "high_severity_rate")
                        else 0
                    )
                    total_exploits = (
                        jvi_data.total_exploits
                        if hasattr(jvi_data, "total_exploits")
                        else 0
                    )
                    total_evaluations = (
                        jvi_data.total_evaluations
                        if hasattr(jvi_data, "total_evaluations")
                        else 0
                    )

                st.metric(
                    "JVI Score", f"{jvi:.2f}", delta=f"{jvi - 50:.1f}" if jvi else None
                )
                st.metric(
                    "Exploit Rate",
                    f"{exploit_rate:.1%}",
                    delta=f"{exploit_rate * 100:.1f}%",
                )
            ...
            with tab1:
                if evaluation_history:
                    ...
                    if trend_fig:
                        st.plotly_chart(
                            trend_fig, use_container_width=True, key="trend_chart"
                        )
                    else:
            ...
            with tab2:
                if evaluation_history:
                    strategy_fig = create_strategy_distribution_chart(
                        evaluation_history
                    )
                    if strategy_fig:
                        st.plotly_chart(
                            strategy_fig, use_container_width=True, key="strategy_chart"
                        )
                    else:
            ...
            with tab3:
                if evaluation_history:
                    ...
                    if severity_fig:
                        st.plotly_chart(
                            severity_fig, use_container_width=True, key="severity_chart"
                        )
                    else:
                        st.info("No successful attacks to analyze")
                else:
            ...
            with tab4:
                if results.get("leaderboard"):
                    ...
                    if isinstance(leaderboard, dict):
                        ...
                        attackers = (
                            leaderboard.top_attackers[:10]
                            if hasattr(leaderboard, "top_attackers")
                            else []
                        )
                    ...
                    if fig:
                        st.plotly_chart(
                            fig, use_container_width=True, key="leaderboard_chart_tab"
                        )
            ...
            with tab5:
                st.markdown(
                    '<h3><i class="fas fa-globe" style="margin-right: 0.5rem;"></i> 3D Vector Space Visualization</h3>',
                    unsafe_allow_html=True,
                )
                st.markdown("**Explore evaluation results in semantic vector space**")
                st.markdown(
                    "Points are colored by attack strategy. Similar responses cluster together."
                )

                if evaluation_history and len(evaluation_history) > 0:
                    # Generate 3D visualization data
                    with st.spinner(
                        "Generating 3D vector embeddings... This may take a moment."
                    ):
                        try:
                                ...
                                if isinstance(eval_data, dict):
                                    ...
                                    try:
                                        ...
                                        eval_result = EvaluationResult(
                                            ...
                                            attacker_id=eval_data.get(
                                                "attacker_id", "unknown"
                                            ),
                                            prompt=eval_data.get("prompt", ""),
                                            defender_id=eval_data.get(
                                                "defender_id", "unknown"
                                            ),
                                            defender_model=eval_data.get(
                                                "defender_model", "unknown"
                                            ),
                                            response=eval_data.get("response", ""),
                                            is_jailbroken=eval_data.get(
                                                "is_jailbroken", False
                                            ),
                            ...
                            output_file = (
                                output_dir
                                / f"vector3d_data_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
                            )

                            data_points = generator.generate_3d_data(
                                eval_results, output_path=output_file, normalize=True
                            )
                            ...
                            if data_points:
                                ...
                                if template_path.exists():
                                    ...
                                    html_file = (
                                        output_dir
                                        / f"vector3d_viewer_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
                                    )
                                    with open(html_file, "w") as f:
                                        f.write(html_content)

                                    st.markdown(
                                        f'<div style="padding: 0.75rem; background: rgba(34, 197, 94, 0.1); border-left: 3px solid rgba(34, 197, 94, 0.8); border-radius: 6px; margin: 0.5rem 0; color: #86efac;"><i class="fas fa-check-circle" style="margin-right: 0.5rem; color: #22c55e;"></i> Generated {len(data_points)} data points for 3D visualization</div>',
                                        unsafe_allow_html=True,
                                    )
                                    ...
                                    try:
                                        ...
                                        components.html(
                                            html_content, height=700, scrolling=False
                                        )
                                    except Exception as e:
                                        st.warning(
                                            f"Could not embed visualization: {e}"
                                        )
                                        st.info(
                                            "Use the download button below to view the HTML file in your browser."
                                        )

                                    # Display instructions
                                    with st.expander(
                                        "How to Use 3D Threat Radar", expanded=False
                                    ):
                                        st.markdown(
                                            """
                                    ...
                                    jailbroken_count = sum(
                                        ...
                                        if p.get("is_jailbroken", False)
                                    )
                                    blocked_count = len(data_points) - jailbroken_count
                                    strategies = len(
                                        set(
                                            p.get("strategy_index", 0)
                                            for p in data_points
                                        )
                                    )
                                    exploit_rate = (
                                        (jailbroken_count / len(data_points) * 100)
                                        if data_points
                                        else 0
                                    )
                                ...
                                st.warning(
                                    "No data points generated. Check evaluation results."
                                )
                    ...
                    st.info(
                        "No evaluation history available for 3D visualization. Run an evaluation first."
                    )
            ...
            with tab6:
                ...
                st.markdown(
                    "*Real-time defense metrics, pattern analysis, and improvement recommendations*"
                )
                ...
                if st.session_state.arena and st.session_state.arena.pattern_database:
                    ...
                    with col_db2:
                        st.metric(
                            "High Severity", analysis.get("high_severity_count", 0)
                        )
                    with col_db3:
                        st.metric(
                            "Critical Patterns", analysis.get("critical_patterns", 0)
                        )
                    with col_db4:
                        st.metric(
                            "Avg Attack Cost",
                            f"{analysis.get('average_attack_cost', 0):.2f}",
                        )
                    ...
                    if analysis.get("strategies"):
                        ...
                        strategy_df = pd.DataFrame(
                            list(strategy_data.items()), columns=["Strategy", "Count"]
                        )
                        strategy_fig = px.bar(
                            strategy_df,
                            x="Strategy",
                    ...
                    try:
                        from src.defense.vulnerability_analyzer import (
                        ...
                        if vulnerabilities:
                            ...
                            st.markdown(
                                f"**Total Vulnerabilities Detected:** {vuln_summary.get('total_vulnerabilities', 0)}"
                            )
                            ...
                            st.info(
                                "No vulnerabilities detected. Defense appears strong!"
                            )
                    except ImportError:
                    ...
                    try:
                        from src.defense.pattern_recognizer import (
                    ...
                    try:
                        ...
                        if rules:
                            ...
                            st.metric(
                                "High Priority",
                                rule_stats.get("high_priority_rules", 0),
                            )
                            ...
                            st.info(
                                "No defense rules generated yet. Run more evaluations to generate rules."
                            )
                    except ImportError:
                    ...
                    st.info(
                        "Pattern database not enabled. Enable it in arena initialization to see defense metrics."
                    )
        ...
        with logs_container:
            with st.expander("Evaluation Log", expanded=False):
                ...
                if evaluation_history:
                    with st.expander(
                        "View Sample Responses (for debugging)", expanded=False
                    ):
                        sample_count = min(5, len(evaluation_history))
                        for i, eval_result in enumerate(
                            evaluation_history[:sample_count]
                        ):
                            # Handle both dict and object access
                            if isinstance(eval_result, dict):
                                ...
                                prompt = (
                                    str(eval_result.prompt)[:150]
                                    if eval_result.prompt
                                    else ""
                                )
                                response = (
                                    str(eval_result.response)[:300]
                            ...
                            st.markdown(
                                f"**Sample {i + 1} - Strategy: {strategy_name}**"
                            )
                            st.markdown(f"**Prompt:** {prompt}...")
                            st.markdown(f"**Response:** {response}...")
                            jailbroken_icon = (
                                '<i class="fas fa-check-circle icon-success" style="margin-right: 0.5rem;"></i> Yes'
                                if is_jailbroken
                                else '<i class="fas fa-times-circle icon-error" style="margin-right: 0.5rem;"></i> No'
                            )
                            st.markdown(
                                f"**Jailbroken:** {jailbroken_icon}",
                                unsafe_allow_html=True,
                            )
                            st.markdown("---")
