...
# ## Multiple Model Configurations - Each model gets its own endpoint
...
MODELS = [
    ...
    {"name": "microsoft/phi-2", "function_name": "serve_phi2"},
    {"name": "tiiuae/falcon-7b-instruct", "function_name": "serve_falcon_7b_instruct"},
    {"name": "Qwen/Qwen-7B-Chat", "function_name": "serve_qwen_7b_chat"},
]
...
async def test(test_timeout=10 * MINUTES, content=None, twice=True, model_index=0):
    """Test a specific model by index (default: 0 for first model)."""
    if model_index >= len(MODELS):
        print(
            f"Error: model_index {model_index} out of range. Available models: {len(MODELS)}"
        )
        return

    model_config = MODELS[model_index]
    serve_func = globals()[model_config["function_name"]]
    url = serve_func.get_web_url()
